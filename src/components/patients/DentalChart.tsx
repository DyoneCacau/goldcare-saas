    ...
    >
      ...
      <div className={cn(
        ...
        <div className="w-full h-full flex items-center justify-center">
          {tooth.status === 'extracted' && <span className="text-lg">✕</span>}
          {tooth.status === 'implant' && <span className="text-xs">🔩</span>}
          {tooth.status === 'prosthesis' && <span className="text-xs">👑</span>}
        </div>
      </div>
...
export function DentalChart({ chart, onUpdateChart, readOnly = false }: DentalChartProps) {
  ...
  const handleStatusChange = (status: ToothStatus) => {
    ...
    toast.success(`Dente ${selectedTooth.number} atualizado para ${TOOTH_STATUS_CONFIG[status].label}`);
  };
  ...
  const handleAddProcedure = () => {
    ...
    const updatedTooth = {
      ...selectedTooth,
      procedures: [...selectedTooth.procedures, procedure],
    };
    ...
    setNewProcedure({ procedure: '', professional: '', date: new Date().toISOString().split('T')[0], notes: '' });
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  {!readOnly && (
                    <Button size="sm" variant="outline" onClick={() => setProcedureDialogOpen(true)}>
                          ...
                          <div className="flex items-start justify-between">
                            ...
                            <div className="text-right">
                              ...
                              <Badge variant="outline" className={cn(
                                ...
                                proc.status === 'scheduled' && 'bg-blue-100 text-blue-700'
                              )}>
                                {proc.status === 'completed' ? 'Concluído' : proc.status === 'pending' ? 'Pendente' : 'Agendado'}
                              </Badge>
                            </div>
                          </div>
                        </Card>
      ...
      <Dialog open={procedureDialogOpen} onOpenChange={setProcedureDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Adicionar Procedimento - Dente {selectedTooth?.number}</DialogTitle>
          </DialogHeader>
